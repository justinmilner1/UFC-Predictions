...
class Preprocessor:
    ...
    def _create_fighter_age(self):
        ...
        #pandas datetime is year-month-day yyyy-mm-dd
        #data is stored as follows:
        # date: month-day.
        #location: year

        #converting location(year) and date to string
        self.store['location'] = self.store['location'].astype(str)
        self.store['date'] = self.store['date'].astype(str)
        print("location: " + str(self.store['location']))
        print("date: " + str(self.store['date']))

        temp = str(self.store['date']).split(" ")
        print("temp: " + str(temp))
        temp[0] = {month: index for index, month in enumerate(calendar.month_abbr) if month}
        print("temp: " + str(temp))
        self.store['date'] = temp
        #month and day swap order halfway through the dataset, need to correct this
        # def swapMonthAndDay(row):
        #     if row['date'][0].isdigit() == False:
        #         datelist = row['date'].split('-')
        #         datelist[0].zfill(2) #adding a zero if needed
        #         temp = datelist[0]
        #         datelist[0] = datelist[1]
        #         datelist[1] = temp
        #         row['date'] = '-'.join(datelist)
        #     else:
        #         datelist = row['date'].split('-')
        #         datelist[1].zfill(2) #adding a zero if needed
        #         row['date'] = '-'.join(datelist)
        #     print("row: " + str(row))
        #     return row

        #self.store['date'] = self.store['date'].apply(swapMonthAndDay, axis=1)



        #combining location(year) and date, then converting to datetime
        self.store['datestring'] = self.store[['date', 'location']].apply(lambda x: '-'.join(x), axis=1)
        print("datestring: " + str(self.store['datestring']))

        self.store["datetime"] = pd.to_datetime(self.store['datestring'], format='%d-%b-%Y') #error: Out of bounds nanosecond timestamp: 1-02-06 00:00:00
        #ValueError: time data '6-Feb-2021' does not match format '%b-%d-%Y'
        #ValueError: time data 'Feb-29-2020' does not match format '%d-%b-%Y' (match)
        def get_age(row):
            B_age = (row["datetime"] - row["B_DOB"]).days
            R_age = (row["datetime"] - row["R_DOB"]).days
        ...
        self.store[["B_age", "R_age"]] = self.store[["datetime", "R_DOB", "B_DOB"]].apply(
            get_age, axis=1
        )
